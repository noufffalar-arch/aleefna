...
import { User, Mail, Globe, LogOut, ChevronRight, Shield } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background pb-24" dir={dir}>
      ...
      <div className="px-6 space-y-4">
        ...
        {isAdmin && (
          <div 
            className="aleefna-card flex items-center gap-4 cursor-pointer hover:bg-secondary/50 transition-colors"
            onClick={() => navigate('/admin')}
          >
            <Shield className="w-5 h-5 text-primary" />
            <span className="flex-1 text-start text-foreground">{t('admin.title')}</span>
            <ChevronRight className="w-5 h-5 text-muted-foreground rtl-flip" />
          </div>
        )}
